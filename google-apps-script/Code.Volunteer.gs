/**
 * Mirani Foundation — Volunteer sign-ups Apps Script Web App
 * -----------------------------------------------------------------------
 * Paste this whole file into Extensions > Apps Script on the Google Sheet
 * you want to use as the volunteer sign-up datastore, then deploy it as a
 * Web App. Full setup steps are in GOOGLE_SHEET_CONTACT_SETUP.md.
 *
 * This is a write-only endpoint (doPost only) — there's no admin page that
 * reads volunteer sign-ups back into the site, so no doGet is needed.
 */
...
/** POST — appends one new volunteer sign-up row. */
function doPost(e) {
  try {
